...
func (e *Example) Call(ctx context.Context, req *api.Request, rsp *api.Response) error {
	...
	if !ok {
		return errors.InternalServerError("go.micro.api.user.example.call", "example client not found")
	}
	...
	if err != nil {
		return errors.InternalServerError("go.micro.api.user.example.call", err.Error())
	}
